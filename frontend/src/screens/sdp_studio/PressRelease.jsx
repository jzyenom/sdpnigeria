...
const PressRelease = () => {
  ...
  const getTitle = (path) => {
    switch (path) {
      ...
      case "/sdp_news_&_update":
        return "SDP News & Updates";
      case "/sdp_&_me":
        return "SDP & Me";
      default:
        return "News list"; // Fallback title
    }
  };
  ...
  return (
    <>
      <section className="page-header @@extraclassNameName">
        ...
        ></div>
        <div className="container">
          {/* Update the h2 tag with the title based on the current link */}
          <h2 className="page-header__title">{getTitle(location.pathname)}</h2>
          <ul className="cleenhearts-breadcrumb list-unstyled">
            <li>
              <i className="icon-home"></i> <a href="/">Home</a>
            </li>
